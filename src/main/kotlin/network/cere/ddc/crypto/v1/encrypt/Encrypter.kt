...
class Encrypter(private val encryptionConfig: EncryptionConfig) {
    private val masterKey = Hex.decode(encryptionConfig.masterKeyHex.removePrefix("0x"))
    private val masterAead = XChaCha20Poly1305(masterKey)
    ...
    fun encrypt(data: ByteArray): ByteArray {
        return when (encryptionConfig.typeHint) {
            TypeHint.JSON -> encryptJson(data)
            TypeHint.RAW -> encryptRaw(data)
        }
    }
